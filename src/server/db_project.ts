import { z } from "zod";
import { db, type DbProject, type DbUser } from "./db.js";
...
export async function getUserProjects(user: DbUser): Promise<DbProject[]> {
    ...
    // const db = getDb();
    const projects = [];
    const allRoles = await db.selectFrom('project_user').selectAll().where('user_uuid', '=', user.uuid).execute();
    for (const role of allRoles) {
        const project = await db.selectFrom('projects').where('uuid', '=', role.project_uuid).selectAll().executeTakeFirstOrThrow();
