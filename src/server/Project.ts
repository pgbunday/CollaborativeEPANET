...
import { db, type DbProject, type DbProjectEdit, type DbUser, type ProjectSnapshotsTable } from "./db";
import { getUserProjects, type DbProjectRoleSchema, type DbProjectSnapshotSchema } from "./db_project";
import { ClientboundPacket, EpanetEdit, LoginSuccessCb, RegisterSuccessCb, TrackEditCb } from "../packets/clientbound";
import { z } from "zod";
...
import { FlowUnits, HeadLossType, Workspace, Project as EpanetProject } from "epanet-js";
...
export class Project {
    ...
    private constructor(dbProject: DbProject, firstClient: WSContext<WebSocket>, snapshot: ProjectSnapshotsTable, edits: DbProjectEdit[]) {
        ...
        this.epanet = EpanetWrapper.fromInp(snapshot.snapshot_inp, dbProject.utm_zone);
        ...
        const chronologicalEdits = this.editTree.getChronologicalWithLastChild(dbProject.current_edit_id);
        for (const edit of chronologicalEdits) {
            this.epanet.applyAction(edit.action);
        }
    }

    public static async getUserRole(projectUuid: string, user: DbUser): Promise<DbProjectRoleSchema | null> {
        const result = await db.selectFrom('project_user').select('role').where('project_uuid', '=', projectUuid).where('user_uuid', '=', user.uuid).executeTakeFirst();
        if (result) {
            return result.role;
        }
        else {
    ...
    public async addDbEdit(edit: EpanetEdit): Promise<boolean> {
        const now = new Date();
        await db.insertInto('project_edits').values({
            created_at: now.toISOString(),
            edit_data: JSON.stringify(edit),
            edit_id: edit.edit_id,
            has_snapshot_file: false,
            project_uuid: this.dbProject.uuid,
            snapshot_id: edit.snapshot_id,
            user_uuid: edit.user_uuid,
        }).execute();
        await db.updateTable('projects').where('uuid', '=', this.dbProject.uuid).set({ current_edit_id: edit.edit_id });
        this.dbProject.current_edit_id = edit.edit_id;
        return true;
    }

    public async incrementEditCount() {
        await db.updateTable('projects').set({ edit_count: this.dbProject.edit_count + 1 }).where('uuid', '=', this.dbProject.uuid);
        this.dbProject.edit_count += 1;
    }

    public static async getDb(projectUuid: string): Promise<DbProject> {
        return await db.selectFrom('projects').selectAll().where('uuid', '=', projectUuid).executeTakeFirstOrThrow();
    }

    public static async addUser(projectUuid: string, user: DbUser, role: DbProjectRoleSchema): Promise<boolean> {
        await db.insertInto('project_user').values({
    ...
    public static async create(name: string, owner: DbUser, longitude: number, latitude: number, zoom: number): Promise<DbProject> {
        const now = new Date();
        ...
        let inp_file = '';
        {
            ...
            project.close();
            await db.insertInto('project_snapshots').values({
                project_uuid: uuid,
                edit_id: 0,
    ...
    public static async connectWsAndSendInit(projectUuid: string, ws: WSContext<WebSocket>): Promise<boolean> {
        ...
        if (maybeExisting) {
            ...
            const { snapshot, edits } = await getSnapshotEditArrays(dbProject, dbProject.current_snapshot_id);
        ...
        // Send init packet
        const { snapshot, edits } = await getSnapshotEditArrays(project.dbProject, project.dbProject.current_snapshot_id);
...
async function getSnapshotEditArrays(dbProject: DbProject, snapshotId: number): Promise<{ snapshot: ProjectSnapshotsTable, edits: DbProjectEdit[] }> {
    const snapshot = await db.selectFrom('project_snapshots').where('project_uuid', '=', dbProject.uuid).where('edit_id', '=', snapshotId).selectAll().executeTakeFirstOrThrow();
    const edits = await db.selectFrom('project_edits').where('project_uuid', '=', dbProject.uuid).where('snapshot_id', '=', snapshotId).where('edit_id', '<>', snapshotId).selectAll().execute();
    return { snapshot, edits }
}

async function getLatestInp(project: DbProject): Promise<string> {
    const { snapshot_inp } = await db.selectFrom('project_snapshots').select('snapshot_inp').where('project_uuid', '=', project.uuid).where('edit_id', '=', project.current_snapshot_id).executeTakeFirstOrThrow();
    return snapshot_inp;
}

async function getEditSnapshotId(dbProject: DbProject, editId: number): Promise<number> {
    const { snapshot_id } = await db.selectFrom('project_edits').where('project_uuid', '=', dbProject.uuid).where('edit_id', '=', editId).select('snapshot_id').executeTakeFirstOrThrow();
    return snapshot_id;
}

async function setEditAndSnapshot(dbProject: DbProject, args: { edit_id: number, snapshot_id: number }) {
    await db.updateTable('projects').set({ current_edit_id: args.edit_id, current_snapshot_id: args.snapshot_id }).where('uuid', '=', dbProject.uuid).execute();
...
export async function handleClientWebSocketMessage(ws: WSContext<WebSocket>, message: MessageEvent<WSMessageReceive>) {
            ...
            try {
                ...
                if (packet.type == "mouse_move_sb") {
                    ...
                    try {
                        ...
                        const chronoEdits = project.editTree.getChronologicalWithLastChild(project.dbProject.current_edit_id);
                        ...
                        project.epanet = EpanetWrapper.fromInp(latest_inp, project.dbProject.utm_zone);
                    ...
                    const snapshotId = await getEditSnapshotId(project.dbProject, packet.edit_id);
                    if (snapshotId == null) {
                        // TODO: notify client that there is no snapshot associated with
                        // that edit. they're doing something funky
                        return;
                    } else if (packet.edit_id == project.dbProject.current_edit_id) {
                        // no op for switching to the same edit
                        return;
                    }
                    if (snapshotId == project.dbProject.current_snapshot_id) {
                        await setEditAndSnapshot(project.dbProject, { edit_id: packet.edit_id, snapshot_id: snapshotId });
                        ...
                        // happens right now.
                        const editArrays = await getSnapshotEditArrays(project.dbProject, snapshotId);
                        if (editArrays == null) {
