...
function runBenchmark() {
	...
	syncState.subscribe([
		(packet) => {
			if (
				packet.type === "epanet_edit_cb" &&
				packet.data.action.type === "add_junction_action"
			) {
				receivedCount += 1;
				if (receivedCount === JUNCTION_COUNT) {
					...
					console.log(
						`Added ${JUNCTION_COUNT} junctions in ${benchMs}ms (${(JUNCTION_COUNT / benchMs) * 1000}/s)`,
					);
				}
			}
		},
...
function handleLoginMessages(msg: MessageEvent<any>) {
	...
	if (packet.type === "login_success_cb") {
		...
		console.log(
			`Project connection took ${connectMs}ms, including password hashing for login`,
		);
