...
import { type MapBrowserEvent, Overlay, Map as UiMap, View } from "ol";
...
export default class MapState {
	...
	public setStyles({
		...
		if (pressureOptions && hydraulicTimeIndex >= 0) {
			...
			for (const [id, properties] of results.nodes) {
				...
				const clamped =
					(pressure.value - pressureOptions.low.value) /
					(pressureOptions.high.value - pressureOptions.low.value);
	...
	private handleCbPacket(p: ClientboundPacket) {
		...
		if (p.type === "track_edit_cb") {
			...
			this.tanksSource = new VectorSource({
				features: new GeoJSON().readFeatures(this.syncedGeoJson.tanks),
			});
			this.tanksLayer.setSource(this.tanksSource);
			this.reservoirsSource = new VectorSource({
				features: new GeoJSON().readFeatures(this.syncedGeoJson.reservoirs),
			});
			this.reservoirsLayer.setSource(this.reservoirsSource);
			this.pipesSource = new VectorSource({
				features: new GeoJSON().readFeatures(this.syncedGeoJson.pipes),
			});
			this.pipesLayer.setSource(this.pipesSource);
			this.cursorsSource = new VectorSource({
				features: new GeoJSON().readFeatures(this.syncedGeoJson.cursors),
			});
			...
			console.warn(
				"Warning: MapState.handleIncomingPacket, unhandled packet:",
				p,
			);
		}
	}
	...
	private renderSbPacket(p: ServerboundPacket) {
		if (p.type === "epanet_action_sb") {
			if (p.data.type === "add_junction_action") {
				...
				this.junctionsSource = new VectorSource({
					features: new GeoJSON().readFeatures(this.localGeoJson.junctions),
				});
				this.junctionsLayer.setSource(this.junctionsSource);
			} else if (p.data.type === "add_pipe_action") {
				this.localGeoJson.addPipe(p.data, this.epanetState.local);
				this.pipesSource = new VectorSource({
					features: new GeoJSON().readFeatures(this.localGeoJson.pipes),
				});
				this.pipesLayer.setSource(this.pipesSource);
			} else if (p.data.type === "add_reservoir_action") {
				this.localGeoJson.addReservoir(p.data);
				this.reservoirsSource = new VectorSource({
					features: new GeoJSON().readFeatures(this.localGeoJson.reservoirs),
				});
				this.reservoirsLayer.setSource(this.reservoirsSource);
			} else if (p.data.type === "add_tank_action") {
				this.localGeoJson.addTank(p.data);
				this.tanksSource = new VectorSource({
					features: new GeoJSON().readFeatures(this.localGeoJson.tanks),
				});
				this.tanksLayer.setSource(this.tanksSource);
			...
			if (clickMode === "add_junction") {
				...
				render(
					<AddJunctionPopup
						...
						applyAndSendChange={(msg) => {
							this.renderSbPacket(msg);
	...
	private handlePipesClick(e: MapBrowserEvent<any>, feature: FeatureLike) {
		...
		if (this.clickMode.getClickMode() === "pan") {
			...
			overlay.setPosition(e.coordinate);
			const id = feature.getProperties().id;
			const { diameter, initial_status, length, loss_coefficient, roughness } =
				this.epanetState.local.getPipeProperties(id);
			render(
				<PipePropertiesPopup
					diameter={diameter}
					id={id}
	...
	private handleReservoirsClick(e: MapBrowserEvent<any>, feature: FeatureLike) {
		if (!e.defaultPrevented) {
			...
			if (clickMode === "add_pipe") {
				try {
					if (this.addPipeState.start_id === "") {
						...
						const toSend: ServerboundPacket = {
							...
							data: this.addPipeState.toAddPipeData(
								this.utmZone,
								this.epanetState.local,
								`P${Math.random()}`,
							),
						};
				...
				const id = feature.getProperties().id;
				const { total_head } =
					this.epanetState.local.getReservoirProperties(id);
				render(
					<ReservoirPropertiesPopup
						applyAndSendChange={(msg) => {
							this.renderSbPacket(msg);
							this.epanetState.applyLocalAndSend(msg);
						}}
		...
		if (!e.defaultPrevented) {
			...
			switch (clickMode) {
				...
				case "pan": {
					...
					const id = feature.getProperties().id;
					const { elevation } =
						this.epanetState.local.getJunctionProperties(id);
					render(
						<JunctionPropertiesPopup
							elevation={elevation}
							id={id}
							project_path={this.project_path}
							applyAndSendChange={(msg) => {
				...
				default:
					console.log(
						"WARNING: unhandled clickMode for junctions-layer:",
						clickMode,
					);
					break;
			}
